...
public class Customer {
	...
	// other class should not involve with these two methods
	...
	private int findMinAndValidQueue(List<FoodStall> foodStallsList, List<FoodStall.Menu> requiredDishes){
		...
		for(int i = 0; i < foodStallsList.size(); i++){
			...
			boolean niceMenu = f.getMenu().containsAll(requiredDishes); // I think this process is computational expensive
			if(
				niceMenu
				&& f.getQueueAble()
			...
			else if(niceMenu && f.getCustomerQueue().size() == 0){
				return i;
			}
		}
		if(anyQueueAble){
